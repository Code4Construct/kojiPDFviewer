...
4. `VERSION` を `0.3.0` のような 3 桁の番号にし、変更をコミットして `master` に push します。
...
2. **Actions → Release kojiPDFviewer MSI → Run workflow** を開き、`master` を選んでリリース名を入力します。例: `kojiPDFviewer v0.3.0`。緑色の **Run workflow** を押します。
